...
QUIZ_DATA = [
    ...
    },
    {
        ...
        "options": [
            "(A) 它僅能處理固定長度的輸入序列",
            "(B) 透過 Query, Key, Value 三個向量計算權重，實現動態關注",
            "(C) 必須依賴 RNN 的遞迴結構才能運作",
            "(D) 只能處理單一語言的文本"
        ]
    },
    {
...
def create_quiz(session, page_id, headers):
    append_url = f"https://api.notion.com/v1/blocks/{page_id}/children"
    session.patch(append_url, json={
        "children": [{
            "object": "block",
            "type": "heading_2",
            "heading_2": {"rich_text": [{"type": "text", "text": {"content": "📅 AI 應用規劃師中級模擬練習 (2026-04-27 v2)"}}]}
        }]
    }, headers=headers)

    for i, data in enumerate(QUIZ_DATA):
        q_text = f"題目 {i+1}：{data['q']}"
        h4_res = session.patch(append_url, json={
            "children": [{
                "object": "block",
                "type": "heading_4",
        ...
        if h4_res.status_code == 200:
            ...
            child_url = f"https://api.notion.com/v1/blocks/{h4_id}/children"
            option_blocks = []
            for opt in data["options"]:
                option_blocks.append({
                    "object": "block",
                    "type": "bulleted_list_item",
                    "bulleted_list_item": {"rich_text": [{"type": "text", "text": {"content": opt}}]}
                })
            option_blocks.append({
                "object": "block",
                "type": "callout",
                "callout": {
                    ...
                    "rich_text": [{"type": "text", "text": {"content": "我的答案：(在此輸入)"}}]
                }
            })
            option_blocks.append({
                "object": "block",
            ...
            session.patch(child_url, json={"children": option_blocks}, headers=headers)
...
def run():
    ...
    print("✍️ 正在重建樹狀結構考題...")
    create_quiz(session, page_id, headers)
    print("\n✨ 考題重建完成！")
