import json
import re

file_a = r"C:\Projects\DGTK\DevelopmentFiles\模型訓練記錄\Jupyter\CYG分文\data\CYG20260428.json"
file_b = r"C:\Projects\DGTK\DevelopmentFiles\模型訓練記錄\Jupyter\CYG分文\data\CYG1108.json"

def extract_rules(data_a):
    rules = []
    for item in data_a:
        judge_way = item.get("判斷方式", "")
        # 僅提取包含 "主旨：" 的判斷方式，作為性質判斷標準
        if "主旨：" in judge_way:
            # 提取主旨之後的關鍵字部分
            match = re.search(r"主旨：([^\\n]+)", judge_way)
            if match:
                keyword = match.group(1).strip()
                rules.append({
                    "keyword": keyword,
                    "target_unit": item.get("實際承辦單位"),
                    "ref_doc_no": item.get("公文文號"),
                    "ref_subject": item.get("主旨"),
                    "ref_agency": item.get("來文機關")
                })
    return rules

def analyze_conflicts():
    with open(file_a, 'r', encoding='utf-8') as f:
        data_a = json.load(f)
    with open(file_b, 'r', encoding='utf-8') as f:
        data_b = json.load(f)

    rules = extract_rules(data_a)
    conflicts = []

    for b_item in data_b:
        b_text = b_item.get("text", "")
        b_actual_unit = b_item.get("label", "") # 根據之前的上下文，CYG1108 的 label 是承辦單位
        b_judge_way = b_item.get("判斷方式", "")
        
        # 如果文件 b 沒有判斷方式，跳過
        if not b_judge_way:
            continue

        for rule in rules:
            # 如果 b 的文本中包含了 a 定義的關鍵字
            if rule["keyword"] in b_text:
                # 比對實際承辦單位是否一致
                if b_actual_unit != rule["target_unit"]:
                    conflicts.append({
                        "a_doc_no": rule["ref_doc_no"],
                        "a_subject": rule["ref_subject"],
                        "a_agency": rule["ref_agency"],
                        "a_unit": rule["target_unit"],
                        "b_actual_unit": b_actual_unit,
                        "b_judge_way": b_judge_way,
                        "b_text_snippet": b_text[:50] + "..."
                    })
                    # 一旦命中一個衝突規則就跳出，避免重複記錄同一筆 b 資料
                    break

    return conflicts

if __name__ == "__main__":
    conflicts = analyze_conflicts()
    print(json.dumps(conflicts, ensure_ascii=False, indent=4))
