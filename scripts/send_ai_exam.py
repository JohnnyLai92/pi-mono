"""
send_ai_exam.py — 讀取 iPAS/*.md，動態產生 AI 應用規劃師模擬考題並寄發 Email + 同步 Notion
...
_STATIC_SETS = [
    # Set A
    [
        "1. [AI 技術應用-NLP] 某電商平台要分析每日數萬則客服對話，自動分類問題類型並偵測負面情緒。下列哪種技術組合最適合？\n   (A) 規則式關鍵字比對 + 固定回應模板\n   (B) 意圖辨識（NLU）+ 情感分析（Sentiment Analysis）\n   (C) 僅使用 N-gram 統計語言模型\n   (D) 圖像辨識 + 語音合成",
        "2. [AI 技術應用 - 電腦視覺] 工廠導入視覺 AI 進行瑕疵品檢測，初期標注資料僅 500 張，訓練後準確率偏低。最合適的改善策略是？\n   (A) 立即採購更昂貴的 GPU 重新訓練\n   (B) 使用資料增強（Data Augmentation）+ 遷移學習（Transfer Learning）\n   (C) 改用純規則式影像處理\n   (D) 增加隱藏層數量至 100 層以上",
        "3. [AI 技術應用 - 生成式 AI] 企業想用生成式 AI 自動撰寫行銷文案，但擔心內容不準確。最有效的風險控制方式是？\n   (A) 完全禁止使用生成式 AI\n   (B) 導入 RAG（檢索增強生成），讓模型從企業知識庫擷取事實依據\n   (C) 提高 Temperature 參數讓輸出更多樣\n   (D) 縮短所有 Prompt 長度",
        ...
        "10. [AI 技術應用 - 導入規劃] 製造業欲導入 AI 視覺檢測取代人工目視，專案規劃師應在哪個階段「定義成功指標（KPI）」？\n   (A) 模型部署並上線後\n   (B) 需求分析與可行性評估階段，與業務部門共同制定\n   (C) 資料收集完成後再決定\n   (D) 等模型訓練完成，看結果再定義",
    ],
    # Set B
    [
        "1. [AI 技術應用 - 多模態] 自駕車系統需同時整合鏡頭影像、雷達點雲與 GPS 訊號進行即時決策。這種整合多種感知來源的 AI 系統屬於哪種技術架構？\n   (A) 單模態深度學習\n   (B) 多模態 AI（Multimodal AI）\n   (C) 純規則式專家系統\n   (D) 強化學習（RL）獨立運作",
        "2. [AI 技術應用-NLP] 企業想建立內部知識問答系統，讓員工查詢公司政策文件。相比直接使用通用 LLM，導入 RAG 架構的主要優勢是？\n   (A) 降低模型推論速度\n   (B) 讓模型能即時存取企業私有文件，減少幻覺並提升答案準確性\n   (C) 完全取代向量資料庫的需求\n   (D) 讓模型參數從零開始訓練",
        ...
        "10. [AI 技術應用 - 風險管理] 某金融機構部署 AI 信貸審核系統後，發現模型在市場劇烈波動期間預測準確率大幅下降。這種現象稱為什麼，應如何應對？\n   (A) 資料過擬合，應增加訓練資料\n   (B) 模型漂移（Model Drift），應建立持續監控機制並定期重新訓練\n   (C) 硬體效能不足，應升級 GPU\n   (D) 超參數設定錯誤，應重新調參",
    ],
    # Set C
    [
        "1. [AI 技術應用 - 生成式 AI] 法律事務所要用 LLM 協助撰寫合約草稿，但擔心模型生成錯誤法條引用。最合適的技術解決方案是？\n   (A) 使用最高 Temperature 值以產生最多元內容\n   (B) 建立法規文件的向量資料庫，搭配 RAG 確保引用內容可溯源\n   (C) 每次人工審閱就不需要任何技術控制\n   (D) 訓練一個新的小型 LLM 從頭學習法律知識",
        ...
        "8. [AI 技術應用 - 導入評估] AI 導入規劃師評估是否導入 AI 進行文件審查時，首要確認的是？\n   (A) 購買最新款 GPU 伺服器\n   (B) 業務痛點、現有流程瓶頸，以及是否有足夠的訓練資料\n   (C) 找最便宜的模型供應商\n   (D) 先做 Demo，之後再評估資料需求",
...
def _generate_with_api(context: str) -> str:
    ...
    prompt = (
        ...
        "2. 各題開頭標註：[科目 - 主題]，科目三選一：AI 技術應用、大數據、機器學習\n"
...
def _static_fallback() -> list:
    """靜態題庫輪替（以星期幾選組，確保週間不重複），回傳題目列表"""
    day_index = datetime.now().weekday() % len(_STATIC_SETS)
    return _STATIC_SETS[day_index]


def _sync_to_notion(questions: list, date_str: str) -> bool:
    """Sync quiz questions to Notion under 'AI 中級應用規劃師題型練習' database."""
    try:
        sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'packages', 'mom', 'src', 'skills'))
        from notion_reader import NotionSkill
        ns = NotionSkill()
        ...
        # Search for database page "AI 中級應用規劃師題型練習"
        search_url = f"{ns.base_url}/search"
        search_payload = {"query": "AI 中級應用規劃師題型練習", "filter": {"value": "page", "property": "object"}}
        
        resp = ns._session.post(search_url, json=search_payload, timeout=10)
        resp.raise_for_status()
        search_results = resp.json().get("results", [])
        
        parent_db_id = None
        for page in search_results:
            title = ns._extract_title(page.get("properties", {}))
            if "AI 中級應用規劃師題型練習" in title:
                # Get the parent database ID from the page's parent
        ...
        if not parent_db_id:
            logger.warning("Parent database 'AI 中級應用規劃師題型練習' not found, skipping Notion sync")
            return False
        
        # Create new page in the database
        create_page_url = f"{ns.base_url}/pages"
        page_title = f"模擬測驗 {date_str}"
        ...
        resp = ns._session.post(create_page_url, json=create_payload, timeout=10)
        resp.raise_for_status()
        new_page = resp.json()
        new_page_id = new_page["id"]
        
        # Add structured content to the new page
        ns.create_quiz_page_structure(new_page_id, questions)
        
        logger.info(f"Notion page created: {page_title} (ID: {new_page_id})")
        return True
...
def main() -> None:
    ...
    if os.getenv("ANTHROPIC_API_KEY") and _IPAS_DIR.exists():
        ...
        if context:
            try:
                ...
                # Parse questions from API response into list
                questions_list = [q.strip() for q in questions_text.split("\n\n") if q.strip()]
            except Exception as e:
                logger.warning("API 出題失敗，改用靜態題庫：%s", e)

    if not questions_list:
        questions_list = _static_fallback()
        questions_text = "\n\n".join(questions_list)
    ...
    # Send Email
    ok_email = send_email(subject=subject, body=body)
    if ok_email:
    ...
    # Sync to Notion
    ok_notion = _sync_to_notion(questions_list, today)
    if ok_notion:
        print(f"Notion sync completed: {today}")
    else:
        print("Notion sync skipped or failed (check logs)")
