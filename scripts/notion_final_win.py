...
class NotionPureCreator:
    ...
    def create_page_minimal(self, database_id, title, sub_items, target_heading):
        ...
        try:
            ...
            # Add Sub-items under target heading
            ...
            # 2. Payload (IMPORTANT: ONLY set the title, skip status to avoid error)
            payload = {
            ...
            if response.status_code == 200:
                print(f"✅ SUCCESS: Page '{title}' created perfectly without status error!")
                return True
            else:
...
if __name__ == "__main__":
    # Use the database ID from earlier
